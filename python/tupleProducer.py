...
class tupleProducer(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch("tau_ipLengthSig", "F")
        
        self.out.branch("tau_decayMode", "I")
        self.out.branch("tau_decayModePNet", "I")
        self.out.branch("tau_hasRefitSV", "F")
        ...
        for _t, _tau in enumerate(taus):
            ...
            if (_tau_v4.Pt()) > 18 and (abs(_tau_v4.Eta()) < 2.3) and (self.deltaR2(signalMu_v4, _tau_v4) > deltaR2Thr):
                ...
                if (self.nanoVer == 10 or self.nanoVer == 11):
                    ...
                    if pass_mva_sel and ( (mva not in best_tau.keys()) ):
                        best_tau[mva] = _tau
    ...
    def selectGenLeg(self, genleptons, dummy=True):
    ...
    def analyze(self, event):
        ...
        if has_reco_tau:
            ...
            self.out.fillBranch("tau_ipLengthSig", tau.ipLengthSig)
            self.out.fillBranch("tau_decayMode", tau.decayMode)
            self.out.fillBranch("tau_decayModePNet", tau.decayModePNet)
            ...
            self.out.fillBranch("tau_ipLengthSig", -999)
            self.out.fillBranch("tau_decayMode", -999)
            self.out.fillBranch("tau_decayModePNet", -999)
